...
const testCases = [
  ...
  {
    name: 'Version bump - Basic patch increment',
    test: () => {
      try {
        ...
        if (versionParts.length !== 3) {
          return { pass: false, result: "Invalid version format" };
        }
        ...
        return { 
          pass: newVersion === "1.2.4", 
          result: { original: testVersion, bumped: newVersion }
        };
      } catch (error) {
        return { 
          pass: false, 
  ...
  {
    name: 'Version bump - Edge case versions',
    test: () => {
      try {
        const testCases = [
        ...
        for (const testCase of testCases) {
          const versionParts = testCase.input.split('.');
          const major = parseInt(versionParts[0]);
          const minor = parseInt(versionParts[1]);
          const patch = parseInt(versionParts[2]) + 1;
          const newVersion = `${major}.${minor}.${patch}`;
          
          if (newVersion !== testCase.expected) {
            return {
              pass: false,
              result: `Failed for ${testCase.input}: expected ${testCase.expected}, got ${newVersion}`
            };
          }
        }
  ...
  {
    name: 'Version bump - Invalid format detection',
    test: () => {
      try {
        const invalidVersions = ["invalid-version", "1.2", "1.2.3.4", ""];
        ...
        for (const version of invalidVersions) {
          ...
          if (versionParts.length === 3) {
            ...
            // If any part is NaN, it's invalid
            if (isNaN(major) || isNaN(minor) || isNaN(patch)) {
              continue; // This is correctly detected as invalid
            } else {
              return {
                pass: false,
                result: `Version ${version} was incorrectly considered valid`
              };
            }
          }
          // If not 3 parts, it's correctly invalid
        }
        
        return { 
          pass: true, 
          result: "Invalid version formats correctly detected"
        };
      } catch (error) {
        return { 
          pass: false, 
